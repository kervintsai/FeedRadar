...
class Program
{
    ...
    static async Task<Product> ScrapeProduct(string url)
    {
        ...
        // find ingredient / 成分 related nodes - look for keywords
        var ingredientNodes = new List<HtmlNode>();
        var keywords = new[] { "成分", "配方", "Ingredients", "主要成分" };
        ...
        foreach (var node in ingredientNodes)
        {
            ...
            foreach (var p in parts)
            {
                ...
                {
                    // ignore headings
                    if (t.Contains("成分") || t.Contains("Ingredients") || t.Contains("配方") || t.Contains("主要成分")) continue;
                    ingredients.Add(t);
                }
            }
        }
